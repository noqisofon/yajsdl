...
public class SDL_RWops extends Structure {
	...
	/**
	 * @param seek Seek to 'offset' relative to whence, one of stdio's whence values:<br>
	 * SEEK_SET, SEEK_CUR, SEEK_END<br>
	 *  Returns the final offset in the data source.<br>
	 * C type : seek_callback<br>
	 * @param read Read up to 'maxnum' objects each of size 'size' from the data<br>
	 *  source to the area pointed at by 'ptr'.<br>
	 *  Returns the number of objects read, or -1 if the read failed.<br>
	 * C type : read_callback<br>
	 * @param write Write exactly 'num' objects each of size 'objsize' from the area<br>
	 *  pointed at by 'ptr' to data source.<br>
	 *  Returns 'num', or -1 if the write failed.<br>
	 * C type : write_callback<br>
	 * @param close Close and free an allocated SDL_FSops structure<br>
	 * C type : close_callback<br>
	 * @param type C type : Uint32<br>
	 * @param hidden C type : hidden_union
	 */
	public SDL_RWops(SDL_RWops.seek_callback seek, SDL_RWops.read_callback read, SDL_RWops.write_callback write, SDL_RWops.close_callback close, int type, hidden_union hidden) {
		super();
		this.seek = seek;
		this.read = read;
		this.write = write;
